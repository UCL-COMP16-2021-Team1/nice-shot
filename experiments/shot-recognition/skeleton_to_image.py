...
def process_skeletons(root, out_dir, _type):
    if not isdir(out_dir):
        mkdir(out_dir)
    
    skeleton_paths = [f for f in listdir(root) if isfile(join(root, f))]
    for s in skeleton_paths:
        skeleton_to_image(root + s, out_dir + _type + "_" + s[:-4] + ".png")
...
if __name__ == "__main__":
    ...
    for s in shot_paths:
        process_skeletons(amateur_root + s + "/", "data/skeleton_images/" + s + "/", "amateur")
    ...
    for s in shot_paths:
        process_skeletons(expert_root + s + "/", "data/skeleton_images/" + s + "/", "expert")
